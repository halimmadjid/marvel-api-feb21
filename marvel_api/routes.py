...
def create_character(current_user_token):
    ...
    character = Character(name,description, comics_appeared_in, super_power, user_id=user_id)
...
def get_character(current_user_token, id):
    ...
    response = character_schema.dump(character) #dump method will take all info and shapeshift from sql table records so that JSON can understand. 
    return jsonify(response)
# UPDATE ENDPOINT
...
def update_character(current_user_token, id):
    ...
    character.name = request.json['name'] #this updates the dabatase
...
@app.route('/google-auth') #the underscore and dash do have to match. it will give an error.
def google_auth(): 
